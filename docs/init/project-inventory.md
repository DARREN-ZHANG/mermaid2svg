...
- `i18n/` — **75 个语言文件**，每个导出一个函数返回翻译对象（含 title, subtitle, formulas_title, benchmark_*, editor_*, usage_*, names[] 等 key）
- `webc/` — Web Components
...
注意：没有 `dev` / `build` / `test` script。开发使用 `bun dev.js`，构建使用 `bun demo/build.js`，测试使用 `./test.sh` 或 `bun test test/compare.test.js`。
...
- 翻译 key 包括：title, subtitle, formulas_title, benchmark_size_*, benchmark_speed_*, editor_*, usage_*, names[]
