...
| 验证                   | READY | bun 与 npm scripts 已补齐，标准验证入口可执行             |
...
| 验证项                          | 结果 | 说明                                           |
| ------------------------------- | ---- | ---------------------------------------------- |
| `./test.sh`                     | PASS | 完整测试链可执行，格式检查为 check-only        |
| `./sh/check.js`                 | PASS | bun shebang 可正常执行                         |
| `bun test test/compare.test.js` | PASS | 421 个 MathML 测试通过                         |
| `npm test` / `npm run build`    | PASS | 标准 npm 入口已补齐，`build` 输出 `demo/dist/` |
...
1. **环境准备任务**: bun 运行时与 `test` / `build` / `extract` npm scripts 已补齐；后续 loop 可直接使用标准入口验证
2. **Human Gate 依赖**: 任务图应标注每个 gate 的依赖关系，相关任务执行到 gate 时暂停
3. **npm scripts 口径**: `build` 指向 demo 静态站点构建，`build:lib` 保留原 library minify 构建
