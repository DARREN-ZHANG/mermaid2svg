...
export function validateSvgOutputPhase(phase: Pick<PhaseDefinition, "id" | "requiredArtifacts">): ValidationResult {
  const errors: string[] = [];
  ...
  if (["compatibility-plan", "normalizer-implementation", "compatibility-tests", "validation", "final-report"].includes(phase.id)) {
    errors.push(...validateRenderInputs());
  }

  if (["normalizer-implementation", "compatibility-tests", "validation", "final-report"].includes(phase.id)) {
...
export function validateCanonicalDocsExist(): string[] {
  ...
  for (const doc of SVG_OUTPUT_LOOP_CONFIG.canonicalDocs) {
    ...
    const fallback = SVG_OUTPUT_LOOP_CONFIG.canonicalDocFallbacks.find((candidate) => path.basename(candidate) === basename);
...
function validateNormalizerEntry(): string[] {
  ...
  for (const required of ["viewBox", "svg", "script", "deterministic"]) {
    if (!source.includes(required)) errors.push(`${file} missing required SVG compatibility concern: ${required}`);
  }
...
function validateCompatibilityReport(): string[] {
  ...
  try {
    ...
    const errors: string[] = [];
    if (!report.summary || typeof report.summary !== "object") errors.push(`${file} must include summary object.`);
    if (!Array.isArray(report.checkedRules)) errors.push(`${file} must include checkedRules array.`);
    if (!Array.isArray(report.failures)) errors.push(`${file} must include failures array.`);
    if (!report.summary.deterministic) errors.push(`${file} summary must record deterministic SVG output status.`);
    return errors;
  } catch (error) {
